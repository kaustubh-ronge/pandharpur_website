      ...
      <div className="relative z-20 flex flex-col h-full min-h-screen items-center justify-center px-4 sm:px-8 md:px-16 lg:px-24 text-center text-gray-800">
        ...
        <div className="md:hidden flex justify-center items-center gap-6 w-full">
          <Image
            ...
            priority
            sizes="50vw"
            className="h-[300px] sm:h-[400px] w-auto mt-4"
          />
          <Image
            src="/rukmini-solo-img.png"
            alt="Mata Rukmini"
            ...
            priority
            sizes="50vw"
            className="h-[280px] sm:h-[380px] w-auto"
          />
        </div>
        ...
        <motion.div
          ...
          className="mb-5"
        >
          <Badge variant="secondary" className="bg-orange-700 text-white text-sm px-4 py-2 shadow-lg">
            Sacred Journey 2025
          </Badge>
        </motion.div>
        ...
        >
          <Link href={'/pandharpur-bookings'}>
            <Button size="lg" className="bg-orange-600 hover:bg-orange-700 text-white shadow-lg">
              <PlayCircle className="mr-2 h-5 w-5" /> Bookings in Pandharpur
            </Button>
          </Link>

          <Link href={'/pandharpur-darshan-yatra-guide'}>
          ...
          <Link href={'https://online.vitthalrukminimandir.org.in/#/login'}>
            <Button size="lg" className="bg-orange-600 hover:bg-orange-700 text-white shadow-lg">
              <Ticket className="mr-2 h-5 w-5" /> Online Pass Booking for Darshan
            </Button>
          </Link>
        </motion.div>
        
    {/* Live Darshan Video Section - Added below buttons */}
{/* <motion.div
  initial={{ opacity: 0, y: 10 }}
  animate={{ opacity: 1, y: 0 }}
...
>
  <div className="relative w-full h-[430px] md:h-[384px] lg:h-[525px] rounded-lg overflow-hidden border-2 border-orange-400 shadow-xl">
    <iframe
      src="http://www.vitthalrukminimandir.org/live_darshan.php"
      className="absolute top-0 left-0 w-full h-[calc(100%+120px)]"  
      style={{ top: "-120px" }}
      title="Live Darshan - Shri Vitthal Rukmini Mandir"
      allowFullScreen
    ...
    />
        ...
        >
          <ArrowDown className="h-8 w-8 text-orange-500 motion-safe:animate-bounce" />
        </motion.div>
      </div>
    </section>
  );
