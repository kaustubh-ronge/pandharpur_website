    ...
    return (
        <div className="relative h-full w-full overflow-hidden rounded-xl">
            ...
            {validImages.length > 1 && (
                <>
                    <button onClick={goToPrev} className="absolute left-3 top-1/2 -translate-y-1/2 bg-orange-500/80 hover:bg-orange-500 text-white rounded-full p-2 shadow-lg transition-colors z-10" aria-label="Previous image">
                        <ChevronLeft className="h-6 w-6" />
                    </button>
                    <button onClick={goToNext} className="absolute right-3 top-1/2 -translate-y-1/2 bg-orange-500/80 hover:bg-orange-500 text-white rounded-full p-2 shadow-lg transition-colors z-10" aria-label="Next image">
            ...
            {validImages.length > 1 && (
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2 z-10">
                    {validImages.map((_, idx) => (
                        <button key={idx} onClick={() => goToImage(idx)} className={`h-2 rounded-full transition-all duration-300 ${idx === currentIndex ? 'w-6 bg-orange-600' : 'w-2 bg-white/90'}`} aria-label={`Go to image ${idx + 1}`} />
                    ))}
                </div>
            )}
            {validImages.length > 1 && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="flex flex-col gap-6">
                    <Card className="bg-white shadow-md border-stone-200 transition-all duration-300 hover:shadow-lg rounded-xl overflow-hidden">
                        ...
                        <CardContent className="space-y-3">
                            <div className="flex items-center gap-3 text-stone-700 text-sm font-medium">
                                <span className="font-bold w-24">Cuisine:</span> <span>{restaurant.cuisineType?.replace(/-/g, ' ')}</span>
                            </div>
                            <div className="flex items-center gap-3 text-stone-700 text-sm font-medium">
                    ...
                    <Card className="bg-white shadow-md border-stone-200 transition-all duration-300 hover:shadow-lg rounded-xl overflow-hidden">
                        ...
                        <CardContent className="space-y-3">
                            {restaurant.mealTypes?.length > 0 ? restaurant.mealTypes.map(meal => (
                            ...
                            )) : <p className="text-stone-500 text-sm">Not specified.</p>}
                        </CardContent>
                    </Card>
                    <Card className="bg-white shadow-md border-stone-200 transition-all duration-300 hover:shadow-lg rounded-xl overflow-hidden">
                        <CardHeader>
                        ...
                        <CardContent className="space-y-3">
                            ...
                            ) : <p className="text-stone-500 text-sm">Phone number not available.</p>}
                        </CardContent>
                    </Card>
                </div>
            </div>
...
// --- NEW MENU SECTION COMPONENT ---
function MenuSection({ restaurant }) {
    if (!restaurant.menuImages?.length && !restaurant.menuPdf) {
        return null;
    }

    return (
        <div className="bg-white rounded-xl shadow-md p-6 md:p-8 border border-stone-200">
            <h2 className="text-3xl font-bold text-stone-800 mb-8 text-center">Our Menu</h2>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                <div className="w-full h-full min-h-[450px] lg:min-h-[550px] rounded-xl overflow-hidden">
                    <GallerySlider images={restaurant.menuImages} restaurantName={`${restaurant.name} Menu`} />
                </div>
                <div className="w-full h-full flex items-center justify-center p-4">
                    {restaurant.menuPdf ? (
                        <Card className="w-full max-w-sm text-center bg-stone-50 border-stone-200 p-8 shadow-inner rounded-xl">
                            <FileText className="h-16 w-16 text-orange-500 mx-auto mb-4" />
                            <CardTitle className="text-2xl font-bold text-stone-800 mb-2">View Full Menu</CardTitle>
                            <p className="text-stone-600 mb-6">Download our complete menu as a PDF for offline viewing.</p>
                            <a href={restaurant.menuPdf} download target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 bg-orange-600 text-white font-bold py-3 px-6 rounded-lg shadow-md hover:bg-orange-700 transition-all duration-300 transform hover:-translate-y-0.5">
                                <Download className="h-5 w-5" />
                                Download PDF
                            </a>
                        </Card>
                    ) : (
                        <div className="flex flex-col items-center justify-center text-center text-stone-500 p-8 h-full bg-stone-50 rounded-xl">
                            <FileText className="h-16 w-16 mx-auto mb-4 opacity-50" />
                            <h3 className="text-xl font-semibold text-stone-700">Menu PDF</h3>
                            <p className="mt-1">A downloadable menu is not available for this restaurant.</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {sites.map((site, index) => (
                        <motion.div key={site.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: index * 0.1 }} viewport={{ once: true }}>
                            <Card className="bg-white border-stone-200 shadow-md transition-all duration-300 hover:shadow-lg h-full rounded-xl overflow-hidden">
                                <CardContent className="flex items-center gap-4 p-6">
                                    <div className="bg-orange-100 p-3 rounded-full">{site.icon}</div>
                                    <div>
                    ...
                    <header>
                        ...
                        <div className="mt-4">
                            ...
                            <div className="mt-4 flex items-center flex-wrap gap-x-4 gap-y-2 text-stone-600">
                                {restaurant.isFeatured && <Badge className="text-base font-bold gap-1.5 py-1.5 px-3 bg-amber-500 text-white border-none rounded-full"><Sparkles className="h-4 w-4 text-yellow-200 fill-current" /> Featured</Badge>}
                                <Badge variant="outline" className="text-base font-medium gap-1.5 py-1.5 px-3 bg-orange-100 text-orange-800 border-orange-300 rounded-full"><Utensils className="h-4 w-4" /> {restaurant.cuisineType}</Badge>
                                <Badge variant="outline" className="text-base font-medium gap-1.5 py-1.5 px-3 bg-green-100 text-green-800 border-green-300 rounded-full"><Tag className="h-4 w-4" /> {priceMap[restaurant.priceIndicator]}</Badge>
                    ...
                    <div className="bg-white rounded-xl shadow-md p-6 border border-stone-200">
                        ...
                        <Tabs defaultValue="menu" className="mt-4">
                            <TabsList className="flex h-auto w-full flex-col rounded-lg bg-stone-100 p-1 sm:w-auto sm:flex-row">
                                <TabsTrigger value="menu" className="w-full justify-center py-2 px-4 data-[state=active]:bg-white data-[state=active]:shadow-sm sm:w-auto">Menu Highlights</TabsTrigger>
                                <TabsTrigger value="contact" className="w-full justify-center py-2 px-4 data-[state=active]:bg-white data-[state=active]:shadow-sm sm:w-auto">Contact & Location</TabsTrigger>
                            </TabsList>
                            ...
                            <TabsContent value="contact" className="mt-6 p-6 bg-stone-50 rounded-lg border border-stone-200">
                                <div className="space-y-4 text-stone-700">
                                    {restaurant.phoneNumber && <p className="flex items-center gap-3 p-3 bg-white rounded-lg border border-stone-200"><Phone className="h-5 w-5 text-orange-500 flex-shrink-0" /> <a href={`tel:${restaurant.phoneNumber}`} className="font-medium hover:underline hover:text-orange-600 transition-colors">{restaurant.phoneNumber}</a></p>}
                                    {restaurant.address && <p className="flex items-center gap-3 p-3 bg-white rounded-lg border border-stone-200"><MapPin className="h-5 w-5 text-orange-500 flex-shrink-0" /> <span className="font-medium">{restaurant.address}</span></p>}
                                </div>
                            </TabsContent>
                        </Tabs>
                    </div>
