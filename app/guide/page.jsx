...
//   });
//   const [loading, setLoading] = useState(true);
//   const [dbUserId, setDbUserId] = useState(null); // Add this state
//   const mapRef = useRef(null);
...
//         // Sync user with Clerk and get database user ID
...
//         setDbUserId(dbUser.id); // Store the database user ID

//         // Check if trip exists using the database user ID
//         const existingTrip = await db.select()
//           .from(trips)
...
//   // Fetch trip data directly from DB
//   const fetchTripData = useCallback(async () => {
//     if (!dbUserId) return; // Use dbUserId instead of userId

//     try {
//       setLoading(true);

//       // Fetch trips and suggestions in parallel using dbUserId
//       const [tripResult, suggestionsResult] = await Promise.all([
//         db.select().from(trips).where(eq(trips.userId, dbUserId)),
//         db.select().from(aiSuggestions).where(eq(aiSuggestions.userId, dbUserId))
//       ]);
...
//       // Update database using dbUserId
//       const result = await db.select().from(trips).where(eq(trips.userId, dbUserId));
//       const currentItinerary = result.length > 0 ? result[0].itinerary || [] : [];
...
//           updatedAt: new Date()
//         })
//         .where(eq(trips.userId, dbUserId));
...
//     try {
//       // Delete from database using dbUserId
//       await db.delete(aiSuggestions).where(eq(aiSuggestions.userId, dbUserId));
...
//       // Update both trips and ai_suggestions tables using dbUserId
//       await Promise.all([
//         db.update(trips)
//           .set({
//             itinerary: generatedEvents,
...
//                         ];
//                         setEvents(events);
//                         // Save to database using dbUserId
//                         db.update(trips)
//                           .set({
//                             itinerary: events,
//                             updatedAt: new Date()
...
export default function TripPlanner() {
  ...
  // Fetch data when dbUserId changes or on initialization
  useEffect(() => {
    if (isInitialized && dbUserId) {
      fetchTripData();
    }
  }, [isInitialized, dbUserId, fetchTripData]);
