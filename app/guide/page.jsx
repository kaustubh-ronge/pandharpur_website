...
export default function TripPlanner() {
  ...
  const initializeTrip = async () => {
    ...
    try {
      ...
      return false; // Trip already existed
    } catch (error) {
      console.error("Trip initialization error:", error);
      toast.error('Failed to initialize trip');
      return false;
    }
  };
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        const isNewTrip = await initializeTrip();
        ...
        const unsubscribe = onSnapshot(
          ...
          (doc) => {
            if (doc.exists()) {
              ...
              if (isNewTrip) {
                toast.success("New trip created! Start adding events");
              }
            }
            setLoading(false);
          },
  ...
  const addEvent = async () => {
    ...
    const eventToAdd = {
      ...
      id: Date.now().toString(),
      addedAt: new Date(),
      date: format(newEvent.date, 'yyyy-MM-dd'), // Ensure consistent date format
    };
